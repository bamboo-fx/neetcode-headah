...
class SingleHeadAttention(nn.Module):
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        output = q@torch.transpose(k, 1, 2)
        context_length, attention_dim = k.shape[1], k.shape[2]
        # masking -> softmax
        # torch.ones of TxT -> torch.tril() fill upper triangle with 0
        # fill upper triangles with -inf
        output = output / (attention_dim ** 0.5)

        tensor = torch.tril(torch.ones((context_length, context_length)))
        mask = tensor == 0
        output = output.masked_fill(mask, float('-inf'))
        final = torch.nn.functional.softmax(output, dim=2)
        return torch.round(final@v, decimals=4)
